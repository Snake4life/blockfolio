...
fs.stat(config.rootPath+"/fetchHistoricalPrices.lock", function(err, stat) {
    if (err == null) {
        console.error("Another fetch is in progress, exiting");
        process.exit();
    } else if (err.code == "ENOENT") {
        // file does not exist
        fs.writeFile(config.rootPath+"/fetchHistoricalPrices.lock", 1);
    } else {
            ...
            request(url, (err, response, body) => {
                ...
                if (data.Response != "Error") {
                    ...
                    requests = requests.filter(
                        el => el.currency.currency_id != currency.currency_id
                    );
                    connection.query(
                        "UPDATE currencies_cryptocompare SET no_market_data = 1 WHERE currency_id = ?",
                        [currency.currency_id]
                    );
                    checkLimits()
                        .then(res => {
