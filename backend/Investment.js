...
module.exports = {
    ...
    arePricesUp2Date: function(userId) {
        return new Promise((resolve, reject)=> {
            ...
            mysql.query(
                "SELECT currency_id,symbol,no_market_data FROM ( SELECT cc.currency_id,tsdiff,cc.symbol,cc.no_market_data,count(ph.currency_id) as count,count(ph.currency_id)=tsdiff as prices_up2date,minDate FROM (SELECT user_id,currency_id,DATE(MIN(i.datetime)) as minDate,timestampdiff(DAY, DATE(MIN(i.datetime)), UTC_TIMESTAMP())+1 as tsdiff FROM investments as i WHERE user_id = ? GROUP BY currency_id) as t1 LEFT JOIN prices_history as ph ON ph.currency_id = t1.currency_id AND ph.date>=t1.minDate LEFT JOIN currencies_cryptocompare as cc ON t1.currency_id = cc.currency_id GROUP BY t1.currency_id ) as x WHERE x.prices_up2date = 0",
                [userId],
                (err, rows, fields) => {
                    if (err) {
                        winston.error(
                        ...
                        if(rows.length>0) return resolve({up_to_date: false, currencies: rows});
                        return resolve({up_to_date: true, currencies: []});
                    }
                }
            );
        });
