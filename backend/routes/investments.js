...
router.get("/currency/:symbol", function(req, res, next) {
    ...
    Currency.getBySymbol(req.params.symbol)
        .then(currency => {
            Investment.getByUserAndCurrencyIdWithBalance(
                req.user.user_id,
...
router.get("/investment/:investmentId", function(req, res, next) {
    ...
    Investment.getByUserAndInvestment(req.user.user_id, req.params.investmentId)
        .then(investment => {
            Currency.getById(investment.currency_id)
                .then(currency => {
...
router.get("/growth", function(req, res, next) {
    ...
    Investment.getGrowthOverTime(req.user.user_id, null)
        .then(response => {
            res.json(response);
        })
        .catch(err => {
...
router.get("/growth/currency/:symbol", function(req, res, next) {
    if (req.user == null) return res.sendStatus(401);
    Currency.getBySymbol(req.params.symbol)
        .then(currency => {
            Investment.getGrowthOverTime(req.user.user_id, currency.currency_id)
                .then(response => {
                    res.json(response);
                })
                .catch(err => {
                    winston.error(
                        "Unable to retrieve investment growth. " + err
                    );
                });
        })
        .catch(err => {
